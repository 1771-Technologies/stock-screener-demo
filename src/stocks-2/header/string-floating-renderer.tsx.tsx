...
import { StringFilterPopoverContent } from "./string-filter-popover/string-filter-popover-content";

export function StringFloatingRenderer({
  column,
  api,
}: Grid.T.HeaderParams<GridSpec>) {
  const filterModel = api.stringFilterModel.useValue();
  const operatorModel = api.stringOperatorModel.useValue();
  const setModel = api.stringSetModel.useValue();
  ...
  }, [filterModel, column.id]);

  return (
    <div className="w-full h-full flex items-center gap-3">
      <Tooltip
        className="border border-gray-300 z-10"
        content={<div className="max-w-[500px]">{contentText}</div>}
        side="bottom"
      >
        <input
          data-ln-input
          ...
          onChange={(e) => {
            ...
            api.stringFilterModel.set((prev) => {
              if (!val.trim()) {
                const next = { ...prev };
      ...
      <Popover open={open} onOpenChange={setOpen}>
        ...
        >
          ...
          {hasFilter && (
            <span className="absolute top-0.5 right-0.5 size-2 rounded-full bg-[var(--ln-primary-50)]" />
          )}
        </Popover.Trigger>
        <Popover.Container>
          <Popover.Arrow />
          ...
          <StringFilterPopoverContent column={column} api={api} />
        </Popover.Container>
      </Popover>
    </div>
  );
