...
export function NumberSetFilter({
  allValues,
  tempSelected,
  setTempSelected,
}: {
  ...
  return (
    <div className="flex flex-col gap-1 min-h-[200px] bg-ln-gray-02 border-ln-gray-30 rounded-lg overflow-hidden">
      ...
      <label className="flex items-center gap-2 px-1 py-0.5 cursor-pointer hover:bg-(--ln-gray-10) rounded text-xs">
        <Checkbox
          checked={allSelected}
        ...
        />
        <span className="text-(--ln-gray-80)">(Select All)</span>
      </label>
      ...
      >
        {filteredValues.map((v: string) => {
          ...
          return (
            <label
              key={v}
              className="flex items-center gap-2 px-1 py-0.5 cursor-pointer hover:bg-(--ln-gray-10) rounded text-xs"
            >
              <Checkbox
                checked={checked}
                onClick={(e) => {
              ...
              />
              <span className="text-(--ln-gray-80) truncate">{v}</span>
            </label>
          );
        })}
      </div>
