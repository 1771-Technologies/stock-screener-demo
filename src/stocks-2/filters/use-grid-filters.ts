...
export function useGridFilters(
  ...
  const { filterFn: stringFilterFn, stringFilterModel } = useStringFilter(columns);
  const { filterFn: numberFilterFn, numberFilterModel } = useNumberFilter(columns);
  const { operatorFilterFn: stringOperatorFn, setFilterFn: stringSetFn, stringOperatorModel, stringSetModel } =
    useStringPopoverFilter(columns, getColumnValues);
  const { operatorFilterFn: numberOperatorFn, setFilterFn: numberSetFn, numberOperatorModel, numberSetModel } =
  ...
  }, [stringFilterFn, numberFilterFn, stringOperatorFn, stringSetFn, numberOperatorFn, numberSetFn]);
